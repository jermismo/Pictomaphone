...
namespace Jermismo.Photo.Imaging
{
    ...
    public struct CieLabT : IPixel
    {
        ...
        public static byte[] col = new byte[1001];
        ...
        static CieLabT()
        {
            ...
            // lookup for sRGB back to RGB
            for (int i = 0; i < 1001; i++)
            {
                double n = i / 1000.0;

                if (n <= 0.0031308) n *= 12.92;
                else n = 1.055 * Math.Pow(n, d1_24) - 0.055;

                col[i] = (n * 255).ClampToByte();
            }
        }
        ...
        /// </summary>
        public CieLabT(float l, float a, float b)
        {
            L = l; A = a; B = b;
        }

        public static CieLabT FromColorB(ColorB col)
        {
            // get CIE XYZ values
        ...
        public ColorB ToColorB()
        {
            ...
            byte red = col[(int)(r * 1000)];
            byte blue = col[(int)(g * 1000)];
            byte green = col[(int)(b * 1000)];

            return new ColorB(red, blue, green);
        }
